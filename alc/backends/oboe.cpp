...
namespace {
    ...
    void OboePlayback::onErrorAfterClose(oboe::AudioStream*, oboe::Result const error)
    {
        ...
        {
            WARN("Oboe stream disconnected after call, reopening...");
            reopenStream();
            return;
        }

        ERR("Error was {}", oboe::convertToText(error));
    }

    void OboePlayback::reopenStream()
    {
        if(mReopening.exchange(true))
            return;
        ...
        std::thread([this]()
                    {
                        ...
                        {
                            ERR("Failed to reopen Oboe stream: {}", e.what());
                        }
